  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        {(Object.entries(STATUS) as [string, { label: string; val: string }][]).map(([s, cfg]) => (
          <button key={s} onClick={() => setFilter(filter === s ? 'all' : s)}
            className={`card p-3 text-left hover:shadow-md transition-all ${filter === s ? 'ring-2 ring-green-600' : ''}`}>
            <p className={`text-2xl font-bold ${cfg.val}`}>{counts[s as keyof typeof counts]}</p>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map(need => {
            const urg = URGENCY[need.urgency];
            return (
