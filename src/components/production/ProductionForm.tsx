...
import { X, Loader2, Package, Ship, ShoppingCart, Info } from 'lucide-react';
...
// Updated status labels: checklist → testes
...
export default function ProductionForm({ production, machines, activeImports = [], onClose, onSuccess }: ProductionFormProps) {
  ...
  useEffect(() => { if (machineId && noStockNoImport) setSource('purchase'); }, [machineId, noStockNoImport]);
  useEffect(() => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!production && !isPurchase && !isImport && !isStock) { setError('Selecione a origem da maquina.'); return; }
    ...
    if (production) {
      const result = await updateProductionStatus(production.id, status, {
        ...
        notes: notes || null, contract: contract || null,
      });
      if (result?.error) { setError(result.error); setLoading(false); return; }
    } else {
      const result = await createProduction({
        machine_id: machineId, contract: contract || null,
        machine_source: isImport ? 'import' : 'stock',
        import_id: importId || null,
        import_po_prosyst: selectedImportObj?.po_prosyst ?? null,
        responsible_mechanical: respMech || null, responsible_electrical: respElec || null,
        responsible_checklist: respCheck || null, responsible_packaging: respPack || null,
        planned_factory_date: planFab || null, planned_delivery_date: planDel || null,
        notes: notes || null,
        machine_code: selectedMachine?.code ?? '', machine_name: selectedMachine?.name,
        qty_physical: selectedMachine?.qty_physical, generate_purchase_need: isPurchase,
      });
      if (result?.error) { setError(result.error); setLoading(false); return; }
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-slide-up">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select value={machineId} onChange={e => setMachineId(e.target.value)} className={inp} required disabled={!!production}>
                ...
                {machines.map(m => <option key={m.id} value={m.id}>{m.code} — {m.name.slice(0, 45)}</option>)}
              </select>
            </div>
            <div>
            ...
            <div className="space-y-3">
                ...
                <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-amber-700 mt-1">Uma <strong>solicitacao de compra urgente</strong> sera gerada para o contrato <strong>{contract || '—'}</strong>.</p>
                    </div>
                  </div>
                  <div className="bg-white border border-amber-200 rounded-lg p-3 flex items-start gap-2">
                    <Info className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
                    <p className="text-xs text-amber-700">Acompanhe na aba <strong>Compras</strong>.</p>
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-3">
                  <button type="button" onClick={() => { setSource('stock'); setImportId(''); setImportMachine(''); }}
                    ...
                    <p className={`text-xs ${hasStock ? 'text-green-600' : 'text-red-500'}`}>
                      {hasStock ? `${selectedMachine?.qty_physical} unidade(s)` : '0 unidades'}
                    </p>
                  </button>
                  <button type="button" onClick={() => hasImport ? setSource('import') : undefined} disabled={!hasImport}
                    className={`p-3 rounded-xl border-2 text-left transition-all ${source === 'import' ? 'border-blue-500 bg-blue-50' : hasImport ? 'border-slate-200 hover:border-slate-300' : 'border-slate-100 bg-slate-50 opacity-60 cursor-not-allowed'}`}>
                    <div className="flex items-center gap-2 mb-1">
              ...
              {source === 'stock' && !noStockNoImport && (
                <div className={`p-3 rounded-lg border text-xs ${hasStock ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-600'}`}>
                  {hasStock ? `✓ ${selectedMachine?.qty_physical} unidade(s). Reserva criada para ${contract || '—'}.` : '⚠ Sem estoque! Solicitacao de compra sera gerada.'}
                </div>
              )}
              ...
              {source === 'import' && !noStockNoImport && (
                <div className="space-y-3 border border-blue-100 rounded-xl p-4 bg-blue-50/30">
                  <div>
                    <label className={lbl}><span className="inline-flex items-center justify-center w-4 h-4 rounded-full text-white text-xs mr-1.5 font-bold" style={{ backgroundColor: '#2563eb' }}>1</span>Selecionar P.O.</label>
                    <select value={importId} onChange={e => { setImportId(e.target.value); setImportMachine(''); }} className={inp}>
                      ...
                      {activeImports.map(imp => (
                        <option key={imp.id} value={imp.id}>
                          {imp.po_prosyst ?? 'Sem P.O.'} — {imp.supplier ?? '—'}{imp.estimated_arrival ? ` · Porto: ${new Date(imp.estimated_arrival).toLocaleDateString('pt-BR')}` : ''} · {imp.items.length} maquina(s)
                        </option>
                      ))}
                    </select>
                  </div>
                  {importId && selectedImportObj && selectedImportObj.items.length > 0 && (
                    <div>
                      <label className={lbl}><span className="inline-flex items-center justify-center w-4 h-4 rounded-full text-white text-xs mr-1.5 font-bold" style={{ backgroundColor: '#2563eb' }}>2</span>Equipamento</label>
                  ...
                  {importId && <div className="p-2 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-700">✓ Reservada para <strong>{contract || '—'}</strong> · P.O. <strong>{selectedImportObj?.po_prosyst ?? '—'}</strong></div>}
                </div>
              )}
            </div>
          )}
          ...
          {/* Responsibles — checklist renamed to testes */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div><label className={lbl}>Resp. Testes</label><input value={respCheck} onChange={e => setRespCheck(e.target.value)} placeholder="Nome" className={inp} /></div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div><label className={lbl}>Data de Entrega</label><input type="date" value={planDel} onChange={e => setPlanDel(e.target.value)} className={inp} /></div>
          </div>
