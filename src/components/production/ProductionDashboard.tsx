...
import { CheckCircle, AlertTriangle, Clock, Activity, Calendar, ChevronRight, TrendingUp } from 'lucide-react';
...
// Override checklist label to "Testes"
const STATUS_LABELS: Record<string, string> = {
  pending:    'Aguardando',
  mechanical: 'Mont. Mecanica',
  electrical: 'Inst. Eletrica',
  checklist:  'Testes',
  packaging:  'Embalagem',
  ready:      'Pronto',
  shipped:    'Despachado',
...
function calcDeliveryDiff(r: Production): number | null {
  // For shipped orders: negative = early, positive = late
  if (r.status !== 'shipped' || !r.planned_delivery_date || !r.actual_delivery_date) return null;
  return Math.ceil((new Date(r.actual_delivery_date).getTime() - new Date(r.planned_delivery_date).getTime()) / 86400000);
}
...
export default function ProductionDashboard({ records }: ProductionDashboardProps) {
  ...
  // Delivered on time (shipped before or on planned_delivery_date)
  const deliveredOnTime  = shipped.filter(r => {
    const diff = calcDeliveryDiff(r);
    return diff !== null && diff <= 0;
  });
  const deliveredEarly   = shipped.filter(r => {
    const diff = calcDeliveryDiff(r);
    return diff !== null && diff < 0;
  });
  ...
  // Average delay of delayed orders
  const avgDelay = delayed.length > 0
    ? Math.round(delayed.reduce((sum, r) => sum + calcDelay(r), 0) / delayed.length)
    : 0;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
        {[
          ...
          { label: 'Entregues no Prazo', value: deliveredOnTime.length,   color: 'text-emerald-600' },
          { label: 'Entregues Antecip.', value: deliveredEarly.length,    color: 'text-teal-600' },
      ...
      {/* Kanban */}
      <div className="card">
        <div className="flex items-center gap-2 px-4 lg:px-5 py-3 border-b border-slate-100">
              ...
              return (
                <div key={status} className="w-32 lg:w-auto shrink-0 lg:shrink">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-slate-400 bg-slate-100 px-1 py-0.5 rounded-full ml-1 shrink-0" style={{ fontSize: '9px', fontWeight: 700 }}>{col.length}</span>
                  </div>
                    ...
                    {col.slice(0, 3).map(r => {
                      ...
                      return (
                        <div key={r.id} className="rounded-lg p-2 border border-slate-100"
                          ...
                          <p className="font-mono font-bold truncate" style={{ fontSize: '10px', color: STAGE_COLORS[status] }}>{r.machine?.code ?? '—'}</p>
                          <p className="truncate text-slate-500" style={{ fontSize: '9px', marginTop: '1px' }}>{r.contract ?? r.machine?.name?.slice(0, 18) ?? '—'}</p>
                          {stageDate && (
                          ...
                          {delay > 0 && <p className="text-red-500 font-bold" style={{ fontSize: '9px' }}>{delay}d atraso</p>}
                        </div>
                      );
                    })}
                    {col.length === 0 && <div className="border border-dashed border-slate-200 rounded-lg p-2 text-center"><p className="text-slate-400" style={{ fontSize: '9px' }}>Vazio</p></div>}
                    {col.length > 3 && <p className="text-center text-slate-400" style={{ fontSize: '9px' }}>+{col.length - 3} mais</p>}
                  </div>
                </div>
              );
            })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="card">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100">
            ...
            {delayed.length > 0 && <span className="ml-auto text-xs font-bold bg-red-100 text-red-700 px-2 py-0.5 rounded-full">{delayed.length}</span>}
          </div>
          {delayedSorted.length === 0 ? (
            ...
            <>
                    ...
                    <div key={r.id} className="flex items-center justify-between px-4 py-3">
                      <div className="min-w-0 mr-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-400">{STATUS_LABELS[r.status]}</span>
                        </div>
                        <p className="text-xs text-slate-500 truncate mt-0.5">{r.contract ?? '—'} · Entrega: {formatDate(r.planned_delivery_date)}</p>
                      </div>
                      <span className={`text-xs font-bold px-2 py-1 rounded-full shrink-0 ${delay > 5 ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{delay}d</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="px-4 py-3 border-t border-slate-100 flex items-center justify-between">
                <span className="text-xs text-slate-500">Media de atraso</span>
                <span className="text-xs font-bold text-amber-600">{avgDelay}d</span>
              </div>
            </>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="divide-y divide-slate-100 max-h-64 overflow-y-auto">
            {active.slice(0, 8).map(r => {
              const stages = [
                ...
                { label: 'Testes', date: (r as any).dt_checklist,  color: '#f59e0b' },
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      {stages.map((s, i) => (
                        <div key={i} className="flex items-center gap-1">
                          ...
                          <span className="text-slate-500" style={{ fontSize: '10px' }}>{new Date(s.date).toLocaleDateString('pt-BR')}</span>
        ...
        <div className="card">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100">
            <TrendingUp className="w-4 h-4 shrink-0" style={{ color: '#008434' }} />
            <h2 className="font-semibold text-slate-800 text-sm">Performance</h2>
          </div>
            ...
            {STATUS_ORDER.map(status => {
              ...
              return (
                <div key={status} className="flex items-center gap-3">
                  <span className="text-xs text-slate-500 w-28 shrink-0">{STATUS_LABELS[status]}</span>
                  <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: STAGE_COLORS[status] }} />
                  </div>
          ...
          <div className="px-4 py-3 border-t border-slate-100 space-y-2">
            {[
              ['Taxa no prazo',       active.length > 0 ? `${Math.round((onTime.length / active.length) * 100)}%` : '—', 'text-emerald-600'],
              ['Entregues no prazo',  shipped.length > 0 ? `${Math.round((deliveredOnTime.length / shipped.length) * 100)}%` : '—', 'text-teal-600'],
              ['Maior atraso',        delayedSorted[0] ? `${calcDelay(delayedSorted[0])}d` : '—', 'text-red-500'],
              ['Media de atraso',     delayed.length > 0 ? `${avgDelay}d` : '—', 'text-amber-600'],
            ].map(([label, value, color]) => (
