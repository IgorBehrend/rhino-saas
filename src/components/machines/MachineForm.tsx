...
export default function MachineForm({ machine, onClose, onSuccess }: MachineFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!code || !name) { setError('Codigo e modelo sao obrigatorios.'); return; }
    ...
    const payload = {
      ...
      qty_system:    qtyPhysical, // keep in sync with physical
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md animate-slide-up">
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Cadastro de maquinas e equipamentos</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="label-base">Modelo de Maquina *</label>
          ...
          <div>
            <label className="label-base">Qtd. Estoque Fisico</label>
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
            ...
            <button type="submit" disabled={loading} className="btn-primary">
              ...
              {machine ? 'Salvar alteracoes' : 'Cadastrar'}
            </button>
          </div>
        </form>
      </div>
